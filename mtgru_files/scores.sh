...
SCORES_TEST_DIR="$SCORES_PYTHON_PATH""test-gwena/"

# Copy files to scores directory
echo "Copy files"
cp $ROOT_DIR$TEST_DIR$TEST_FILE_OUT_1 "$SCORES_TEST_DIR"
cp $ROOT_DIR$TEST_DIR$TEST_FILE_OUT_2 "$SCORES_TEST_DIR"


# Run python file to get BLEU and ROUGE scores
echo "Get scores"
HYP_FILE="$SCORES_TEST_DIR"$TEST_FILE_OUT_1
REF_FILE="$SCORES_TEST_DIR"$TEST_FILE_EXP_1
SCORES_FILE_1="$SCORES_TEST_DIR"$SCORES_FILE_1
python "$SCORES_PYTHON_PATH""tester_allSentencesOneFile.py" --hyp_file="$HYP_FILE" --ref_file="$REF_FILE" --scores_file="$SCORES_FILE_1"

HYP_FILE="$SCORES_TEST_DIR"$TEST_FILE_OUT_2
REF_FILE="$SCORES_TEST_DIR"$TEST_FILE_EXP_2
SCORES_FILE_2="$SCORES_TEST_DIR"$SCORES_FILE_2
