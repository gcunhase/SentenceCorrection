...
class MTGRUCell(RNNCell):
  ...
  def __init__(self, num_units, input_size=None, tau = 1.0, activation=tanh):
    if input_size is not None:
      logging.warn("%s: The input_size parameter is deprecated.", self)
    self._num_units = num_units
    self._activation = activation
    ...
    with vs.variable_scope(scope or "gru_cell"):
      ...
      with vs.variable_scope("candidate"):
        c = self._activation(_linear([inputs, r * state],
                                     self._num_units, True,
                                     scope=scope))
      new_h = (u * state + (1 - u) * c)*(self._tau) + (1-self._tau)*state
    return new_h, new_h
...
class MTGRUCell(RNNCell):
  ...
  def __init__(self, num_units, input_size=None, tau = 1.0, activation=tanh, reuse=None):
