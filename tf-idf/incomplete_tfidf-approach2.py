...
#Simple
PERCENTAGE_ALLOW_MISSING_WORD = 0.1 #0.8 #0.5 #0.4
PERCENTAGE_ALLOW_MISSING_SUBSTRING = 0.5 #0.6
#Complex
...
def save_dict_fromMultipleFiles_on_pickle_and_file(data_dir, cnn_dir, dailymail_dir):
    ...
    table.save_dictionary(data_dir+DICT_FILENAME, "both") #"pickle")#"both")
    return new_data_dir
...
'''
    Real TF-IDF
'''
def get_datasets_fromMultipleFiles_missing_simpleOrComplex_words_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, cnn_dir, dailymail_dir, simpleOrComplexType):
    print("Dataset "+simpleOrComplexType.upper()+" words - multiple files - percentage")
    
    new_data_dir = get_new_data_dir_name(data_dir, "-missing"+simpleOrComplexType+"-"+str(percentage_missing_words_per_file)+"perc")
    get_datasets_fromMultipleFiles_missing_words_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, new_data_dir, cnn_dir, dailymail_dir, simpleOrComplexType)

def get_datasets_fromMultipleFiles_missing_words_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, new_data_dir, cnn_dir, dailymail_dir, simpleOrComplexType):
    # Run through all files in files_cnn  
    if not os.path.exists(new_data_dir+cnn_dir):
        os.makedirs(new_data_dir+cnn_dir)
    print("Modifying CNN dataset...")  
    get_missing_dataset_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, new_data_dir, cnn_dir, simpleOrComplexType)
    
    # Run through all files in files_dailymail  
    if not os.path.exists(new_data_dir+dailymail_dir):
        os.makedirs(new_data_dir+dailymail_dir)
    print("Modifying Daily Mail dataset...")
    get_missing_dataset_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, new_data_dir, dailymail_dir, simpleOrComplexType) 


def get_missing_dataset_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, new_data_dir, stories_dir, simpleOrComplexType):
    files = [f for f in os.listdir(data_dir+stories_dir) if f.endswith('.story')]
    all_files_log_info = ""
    for i in range(0, len(files)):
       filename = files[i] 
       ...
       if (simpleOrComplexType == "simple"):
           sorted_doc_inv_x.reverse()
...
def get_datasets_missing_words_from_sentences_fixedRatioPercentage(data, percentage_missing_words_per_file, words_in_doc, sorted_doc_inv_x):
    ...
    len_doc = len(sorted_doc_inv_x)
    words_to_be_missing = int(min(floor(percentage_missing_words_per_file*len_doc), len_doc))
    #print("Words in doc: "+str(words_in_doc)+" - words_to_be_missing: "+str(words_to_be_missing)+" - Length sorted_doc: "+str(len_doc))
    allow_missing = 1
    count = 0
...
def multiple_files():
    ...
    ### Simple dataset
    #print("Making simple dataset...")
    ##Percentage from entire document
    ##top_words = 100
    ##get_datasets_fromMultipleFiles_missing_simple_words(sorted_x, top_words, data_dir, cnn_dir, dailymail_dir)
    ##Percentage from each file
    #percentage_missing_words_per_file = 0.01
    #docs_dic_dir = documents[0][1]
    #get_datasets_fromMultipleFiles_missing_simpleOrComplex_words_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, cnn_dir, dailymail_dir, "simple")


    ## Complex dataset
    print("Making complex dataset...")
    percentage_missing_words_per_file = 0.6
    docs_dic_dir = documents[0][1]
    get_datasets_fromMultipleFiles_missing_simpleOrComplex_words_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, cnn_dir, dailymail_dir, "complex")

    percentage_missing_words_per_file = 0.8
    docs_dic_dir = documents[0][1]
    get_datasets_fromMultipleFiles_missing_simpleOrComplex_words_fixedRatioPercentage(percentage_missing_words_per_file, data_dir, docs_dic_dir, cnn_dir, dailymail_dir, "complex")
